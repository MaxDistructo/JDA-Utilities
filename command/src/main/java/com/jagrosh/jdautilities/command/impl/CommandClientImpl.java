...
import maxdistructo.discord.core.blacklist.commands.CommandBan;
import maxdistructo.discord.core.blacklist.commands.CommandUnban;
...
public class CommandClientImpl implements CommandClient, EventListener {
    ...
    public CommandClientImpl(String ownerId, String[] coOwnerIds, String prefix, String altprefix, Activity activity, OnlineStatus status, String serverInvite,
        ...
        //START BLACKLIST PATCH
        commands.addAll(Arrays.asList(new CommandBan(), new CommandUnban()));
        //END BLACKLIST PATCH
        // Load commands
        for (Command command : commands) {
            addCommand(command);
    ...
    private void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (parts != null) //starts with valid prefix
        {
            if (useHelp && parts[0].equalsIgnoreCase(helpWord)) {
                ...
                //START BLACKLIST PATCH
                if (Blacklist.checkId(event.getMessage().getAuthor().getIdLong())) {
                    System.out.println("[MDPATCH] - Running in blacklist check.");
                    event.getChannel().sendMessage("You have been banned from running commands on this bot.").queue();
                    long timeFail = Blacklist.getBanLeft(event.getMessage().getAuthor().getIdLong());
                    if (timeFail != -1L) {
                        event.getChannel().sendMessage("You will be unbanned in " + Blacklist.calculateTime(timeFail) + ".").queue();
                    } else {
                        event.getChannel().sendMessage("You are currently permbanned. Please appeal with <@!" + ownerId + ">").queue();
                    }
                } else {
                //END BLACKLIST PATCH
                    if (command != null) {
